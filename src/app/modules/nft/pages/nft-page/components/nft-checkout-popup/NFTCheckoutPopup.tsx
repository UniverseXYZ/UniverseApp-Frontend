...
export const NFTCheckoutPopup = () => {
  ...
  const { setShowError, setErrorBody } = useErrorStore((s) => ({
    setShowError: s.setShowError,
    setErrorBody: s.setErrorBody,
  }));
  const { NFT, collection, NFTs, order, isOpen, closeCheckout } =
    useNftCheckoutStore();
  ...
  const [newOrderInfo, setNewOrderInfo] =
    useState<IOrder<IOrderAssetTypeSingleListing, IOrderAssetTypeERC20>>();
  ...
  const prepareMutation = useMutation<
    AxiosResponse<any>,
    AxiosError<any>,
    IPrepareMutationData
  >(({ hash, data }) => {
    return axios.post(
      `${process.env.REACT_APP_MARKETPLACE_BACKEND}/v1/orders/${hash}/prepare`,
      data
    );
  });
  ...
  const handleCheckoutClick = useCallback(async () => {
    try {
      ...
      const orderIndexing = setInterval(async () => {
        ...
        const singleListingOrder = order as IOrder<
          IOrderAssetTypeSingleListing,
          IOrderAssetTypeERC20
        >;
      ...
      const nftIndexing = setInterval(async () => {
        ...
        if (
          ...
          newNft._ownerAddress?.toLowerCase()
        ) {
      ...
      if (
        ...
        !!prepareMutation?.error?.response?.data?.message
      ) {
  ...
  const sendSellTransaction = async (
    data: string,
    from: string,
    to: string,
    value: BigNumber
  ...
  const listingPrice = useMemo(() => {
    return Number(
      utils.formatUnits(order?.take?.value || 0, priceToken.decimals)
    );
  }, [order, priceToken]);
  ...
  useEffect(() => {
    if (isOrderIndexed && isNftIndexed) {
      ...
      queryClient.invalidateQueries(
        orderKeys.listing({ tokenId, collectionAddress })
      );
      queryClient.invalidateQueries(
        orderKeys.history({ tokenId, collectionAddress })
      );
      ...
      if (newNftInfo) {
        queryClient.setQueryData(
          nftKeys.nftInfo({ tokenId, collectionAddress }),
          newNftInfo
        );
      } else {
  ...
  if (!previewNFT || !NFT?.tokenId || !order?._id) {
    return null;
  }
          ...
          {state === CheckoutState.CHECKOUT && (
            <>
              ...
              <HStack spacing={"16px"} {...s.NFTWrapper}>
                <Box pos={"relative"}>
                  <Box {...s.AssetStyle}>
                    {(isNFTAssetImage(previewNFT.artworkTypes) && (
                      <Image src={previewNFT.thumbnailUrl} />
                    )) ||
                      (isNFTAssetVideo(previewNFT.artworkTypes) && (
                        <video src={previewNFT.thumbnailUrl} />
                      )) ||
                      (isNFTAssetAudio(previewNFT.artworkTypes) && (
                        <Image src={AudioNFTPreviewImage} />
                      ))}
                  </Box>
                </Box>
                <Box flex={1}>
                  ...
                  <Text {...s.SecondaryText}>
                    {collection?.name ||
                      shortenEthereumAddress(NFT?._collectionAddress)}
                  </Text>
                </Box>
          ...
          {state === CheckoutState.PROCESSING && (
            <Box>
              ...
              <Text
                ...
                textAlign={"center"}
              >
                Just accept the signature request and wait for us to process
                your offer
              </Text>
              ...
              {(fetchOrderCount >= 3 && !isOrderIndexed) ||
                (fetchNftCount >= 5 && !isNftIndexed && (
                  <Text
                    ...
                    textAlign={"center"}
                  >
                    Receving the events from the blockchain is taking longer
                    than expected. Please be patient.
                  </Text>
                ))}
            ...
            <Box>
              ...
              {approveTx && (
                ...
                >
                  ...
                  <a
                    target="_blank"
                    href={getEtherscanTxUrl(approveTx)}
                    rel="noreferrer"
                    style={{ color: "blue" }}
                  >
          ...
          {state === CheckoutState.CONGRATULATIONS && (
            <>
              ...
              <Box {...s.AssetCongratsStyle}>
                ...
                {!!NFTs && !!NFTs?.length && (
                  <NFTType type={"bundle"} count={NFTs.length} />
                )}
              </Box>

              <HStack spacing={"16px"} justifyContent={"center"} mt={"32px"}>
                <Button boxShadow={"lg"} onClick={handleMyNFTsClick}>
                  My NFTs
                </Button>
