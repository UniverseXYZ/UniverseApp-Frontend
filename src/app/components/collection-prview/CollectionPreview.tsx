...
const formatPrice = (price: string) => {
  return Number(price) < 0.001 ? '< 0.001' : price && price.length > 7 ? `> ${price.substring(0, 5)}` : price
}

interface ICollectionPreviewProps {
  collection: ICollection | string | null;
  children: React.ReactChild | ((collection: ICollection | null) => React.ReactChild);
}
...
export const CollectionPreview = (props: ICollectionPreviewProps) => {
  ...
  const { data: collection, isLoading: isLoadingCollection } = useQuery(
    ...
    {
      retry: false,
      enabled: typeof initialCollection === 'string',
      initialData: initialCollection && initialCollection !== 'string'
  ...
  const { data: collectionOwner } = useQuery(
    ...
    async () => {
      try {
        const network = providers.getNetwork(+`${process.env.REACT_APP_NETWORK_CHAIN_ID}`);
        const provider = providers.getDefaultProvider(network);

        // @ts-ignore
        const { abi } = Contracts[process.env.REACT_APP_NETWORK_CHAIN_ID].contracts.UniverseERC721Core;
  ...
  const { data: collectionStatistic } = useQuery(
    ...
    async () => {
      const [NFTsResponse, ownersResponse, orderResponse] = await Promise.allSettled([
        GetCollectionNFTsApi(utils.getAddress(`${collection?.address.toLowerCase()}`), 1, 1, ''),
        GetCollectionGeneralInfo(`${collection?.address}`),
        GetCollectionOrderBookData(`${collection?.address}`),
      ]);

      return {
        NFTs: NFTsResponse.status === 'fulfilled' ? NFTsResponse.value.total : 0,
        owners: ownersResponse.status === 'fulfilled' ? ownersResponse.value.owners : '-',
        floorPrice: orderResponse.status !== 'fulfilled' || parseFloat(orderResponse.value.floorPrice) === 0
          ? null
          : formatPrice(utils.formatEther(orderResponse.value.floorPrice)),
        volumeTraded: orderResponse.status !== 'fulfilled' || parseFloat(orderResponse.value.volumeTraded) === 0
          ? null
  ...
  const renderCollection = useCallback(() => {
    return (
      <Flex flexDir={'column'}>
        <Box {...styles.getBannerImageStyles(collection?.bannerUrl, collection?.address)} />
        <Center {...styles.getCoverImageStyles(collection?.coverUrl, collection?.address)}>
          {!collection?.coverUrl && collection?.name ? <Text>{collection?.name.charAt(0)}</Text> : null}
        </Center>
            ...
            <Box {...styles.StatisticGridItem}>
              ...
              <Text {...styles.StatisticGridItemValue}>
                {!collectionStatistic?.floorPrice ? '-' : (
                  <>
                    <Image src={EthIcon} alt={'Ethereum icon'} {...styles.EthereumIcon} />
                    {collectionStatistic?.floorPrice}
                  </>
                )}
              </Text>
