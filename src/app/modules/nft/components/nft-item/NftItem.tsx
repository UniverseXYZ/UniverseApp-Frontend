...
export const NftItem = ({
  ...
  const { data: NFT, isLoading: isLoadingNFT } = useQuery(
    nftKeys.nftInfo({
      collectionAddress: _NFT._collectionAddress || "",
  ...
  const [
    ...
    const bestOfferPriceT = !data?.bestOffer
      ? null
      : getTokenByAddress(
          (data?.bestOffer.make.assetType as IERC721AssetType).contract
        );
    const bestOfferP = !bestOfferPriceT
      ? null
      : utils.formatUnits(
          data?.bestOffer.make.value || 0,
          bestOfferPriceT.decimals ?? 18
        );
    ...
    if (data && data.lastOffer && data?.lastOffer.side) {
      switch (data?.lastOffer.side) {
        // Sell order
        case 0:
          lastOfferPriceT = !data?.lastOffer
            ? null
            : getTokenByAddress(
                (data?.lastOffer.make.assetType as IERC721AssetType).contract
              );
          lastOfferP = !lastOfferPriceT
            ? null
            : utils.formatUnits(
                data?.lastOffer.make.value || 0,
                lastOfferPriceT.decimals ?? 18
              );
          break;
        // Buy order
        case 1:
          lastOfferPriceT = !data?.lastOffer
            ? null
            : getTokenByAddress(
                (data?.lastOffer.take.assetType as IERC721AssetType).contract
              );
          lastOfferP = !lastOfferPriceT
            ? null
            : utils.formatUnits(
                data?.lastOffer.take.value || 0,
                lastOfferPriceT.decimals ?? 18
              );
          break;
      }
    ...
    >
      <LinkBox>
        <NextLink
          href={
            !onClick
              ? `/nft/${NFT?._collectionAddress}/${NFT?.tokenId}`
              : "javascript: void(0);"
          }
        >
          <LinkOverlay display={"contents"}>
            {!NFT || renderHeader === null
              ? null
              : renderHeader
              ? renderHeader({
                  NFT: NFT as INFT,
            ...
            >
              {!NFT || renderAsset === null ? null : renderAsset ? (
                renderAsset({
                  NFT: NFT as INFT,
                  collection: collection as ICollection,
                  creator,
                ...
                <NFTItemAsset
                  NFT={NFT}
                  orderEnd={orderEnd || orderData?.end || 0}
                  onAuctionTimerEnd={() =>
                    onAuctionTimerEnd && onAuctionTimerEnd()
                  }
                />
              )}
            </Box>
            <Box {...styles.NFTContentStyle}>
              {!NFT || renderContent === null ? null : renderContent ? (
                renderContent({
                  NFT: NFT as INFT,
                  collection: collection as ICollection,
                  creator: creator,
                  ...
                  <Box mb={"14px"}>
                    <Flex>
                      <NFTItemRelation
                        ...
                        linkParam={
                          creator?.artist?.universePageAddress ||
                          NFT._creatorAddress?.toLowerCase() ||
                          ""
                        }
                      />
                      <NFTItemRelation
                      ...
                      <NFTItemRelation
                        ...
                        linkParam={
                          owner?.artist?.universePageAddress ||
                          owner?.address ||
              ...
              {!NFT || renderFooter === null ? null : renderFooter ? (
                renderFooter({
                  NFT: NFT as INFT,
                  collection: collection as ICollection,
                  creator,
                ...
                <NFTItemFooter
                  ...
                  showBuyNowButton={
                    showBuyNowButton &&
                    (orderData || order) &&
                    (orderData || order)?.side === OrderSide.SELL &&
                    (orderData || order)?.status === OrderStatus.CREATED
                      ? true
                      : false
                  }
                  NFT={NFT as INFT}
